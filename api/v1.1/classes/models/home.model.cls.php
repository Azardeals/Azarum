...
class HomeClass extends ModelClass
{
    ...
    public function homePageInfo($args)
    {
        ...
        $homePage = [];
        $data = fetchBannerDetail(4, 5);
        $banner = [];
        $companydata = [];
        $top_products = [];
        foreach ($data as $key => $info) {
            $banner[$key]['url'] = BANNER_IMAGES_URL . $info['banner_image'];
            $banner[$key]['linked_url'] = $info['banner_url'];
            $banner[$key]['banner_target'] = $info['banner_target'];
        }
        $homePage['bannerInfo'] = $banner;
        if ($rs = fetchTopVendors($city, 5)) {
        ...
        if ($rsltset = fetchTopProducts(4, $city)) {
        ...
        $deal_info = [];
        $homePage['featured_deals'] = $this->getFeaturedDeals(1, 8, $city);
    ...
    private function getFeaturedDeals($page, $pagesize, $cityId = 0)
    {
        global $db;
