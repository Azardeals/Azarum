...
class MerchantClass extends ModelClass
{
    ...
    public function getReviews($args)
    {
        ...
        while ($row = $db->fetch($rs)) {
            if ($row['reviews_approval'] == 1) {
                $reply = getcompanyReviewReply($row['reviews_id']);
                $record['reviews_id'] = $row['reviews_id'];
                $reviews_reviews = filter_var(html_entity_decode($row['reviews_reviews'], ENT_COMPAT, 'UTF-8'), FILTER_SANITIZE_STRING);
                $reviews_reviews = html_entity_decode($reviews_reviews, ENT_QUOTES | ENT_XML1, 'UTF-8');
                $record['reviews_reviews'] = str_replace(["&nbsp;"], "", $reviews_reviews);
                ...
                $record['reviews_is_replied'] = !($reply) ? 0 : 1;
                $record['reviews_reply'] = !($reply) ? [] : array($reply);
                $data[] = $record;
            }
        }
    ...
    public function fetchFavoriteMerchantList($args)
    {
        ...
        while ($companyrow = $db->fetch($rs_listing)) {
            ...
            $companyrow['company_reviews'] = $companyrow['reviews'];
            if (CONF_REVIEW_RATING_MERCHANT) {
                $compRating = fetchCompanyRating($companyrow['company_id']);
                $companyrow['company_rating'] = $compRating['rating'];
                $companyrow['rating'] = $compRating['rating'];
            }
            $vendors[] = $companyrow;
        }
