...
$arr_bread = [
    ...
    'representative.php' => t_lang('M_TXT_REPRESENTATIVE'),
    '' => t_lang('M_TXT_COMMISSION_EARNINGS')
];
?>
</div></td>
<td class="right-portion"><?php echo getAdminBreadCrumb($arr_bread); ?>
    <div class="div-inline">
        <div class="page-name"><?php echo t_lang('M_TXT_COMMISSION_EARNINGS'); ?></div>
    </div>
    <div class="clear"></div>
    ...
    <table class="tbl_data" width="100%">
        <thead>
            <tr>
                <th width='50%'><?php echo t_lang('M_TXT_COMPANY'); ?></th>
        ...
        <tbody>
            ...
            if ($total_records != 0) {
                ...
                while ($row = $db->fetch($wallet_data)) {
                    ...
                    $srch = new SearchBase('tbl_companies', 'c');
                    /* $srch->addCondition('c.company_rep_id', '=', $rep_id, 'OR') */
                    $srch->addCondition('c.company_id', '=', $row['company_id'], 'OR');
                    $srch->joinTable('tbl_deals', 'INNER JOIN', 'c.company_id=d.deal_company', 'd');
                    $srch->addMultipleFields(array('c.company_id', 'd.deal_id', 'd.deal_company'));
                    ...
                    if ($db->total_records($repDeal) > 0) {
                        ...
                        echo '<td>&nbsp;';
                        if ($db->total_records($data) > 0 && $amountRow['totalAmount'] > 0) {
                            echo '<a href="sales.php?deal_company=' . $amountRow['deal_company'] . '&uid=' . $_GET['uid'] . '" >' . CONF_CURRENCY . number_format($amountRow['totalAmount'], 2) . CONF_CURRENCY_RIGHT . '</a>';
                        } else {
                            echo '<a href="sales.php?deal_company=' . $company['0'] . '&uid=' . $_GET['uid'] . '" >' . CONF_CURRENCY . '0.00' . CONF_CURRENCY_RIGHT . '</a>';
                        }
                        $totalAmount += $amountRow['totalAmount'];
                        echo '</td>';
                    } else {
                        echo '<td>&nbsp;';
                        echo '<a href="sales.php?deal_company=' . $row['company_id'] . '&uid=' . $_GET['uid'] . '" >' . CONF_CURRENCY . '0.00' . CONF_CURRENCY_RIGHT . '</a>';
                        echo '</td>';
                    }
                    echo '<td>&nbsp;';
                    if (!empty($deal)) {
                ...
                echo '<tr><td colspan=>' . t_lang('M_TXT_TOTAL') . '</td><td>' . CONF_CURRENCY . $totalAmount . CONF_CURRENCY_RIGHT . '</td><td>' . $total_rep_commission . '</td></tr>';
            } else {
