...
$fld = $frm->addDateField(t_lang('M_FRM_STARTS_TIME'), 'cwh_start_time', '', 'cwh_start_time', 'readonly');
$fld->html_before_field = "<div class='frm-dob fld-req'>";
$fld->html_before_field = "</div>";
$fld->requirements()->setRequired(true);
$fld = $frm->addDateField(t_lang('M_FRM_END_TIME'), 'cwh_end_time', '', 'cwh_end_time', 'readonly');
$fld->html_before_field = "<div class='frm-dob fld-req'>";
$fld->html_before_field = "</div>";
$fld->requirements()->setRequired(true);
...
$srch->addMultipleFields(array('cm_counpon_no', 'deal_paid', 'deal_status', 'deal_bonus', 'deal_id', 'deal_name' . $_SESSION['lang_fld_prefix'] . ' as deal_name', 'c.company_name' . $_SESSION['lang_fld_prefix'] . ' as company_name'));
$srch->addCondition('deal_paid', '!=', 1);
if (isset($post['btn_submit'])) {
    $company_id = intval($_GET['company_id']);
    $data = array('company_id' => $company_id, 'cwh_start_time' => $post['cwh_start_time'], 'cwh_end_time' => $post['cwh_end_time']);
    $frm->fill($data);
    if (!empty($company_id)) {
        $srch->addCondition('deal_company', '=', $company_id);
    }
    if (!empty($post['cwh_start_time']) && !empty($post['cwh_end_time'])) {
        $srch->addCondition('o.order_date', 'BETWEEN', array(date('Y-m-d', strtotime($post['cwh_start_time'])), date('Y-m-d', strtotime($post['cwh_end_time']))));
    }
}
...
<td class="right-portion">
    ...
    $arr_bread = [
        ...
        'javascript:void(0)' => t_lang('M_TXT_DEAL_UNSETTELED_REPORT'),
    ...
    <div class="div-inline">
        <div class="page-name"><?php echo t_lang('M_TXT_DEAL_UNSETTELED_REPORT'); ?> </div>
    </div>
    <div class="clear"></div> 
    <div class="box searchform_filter">
        <div class="title"><?php echo t_lang('M_TXT_DEAL_UNSETTELED_REPORT'); ?> </div>
