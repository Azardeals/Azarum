...
function displayMsg(order_id) {

    document.getElementById(order_id).style.backgroundColor = '#bcbcbc';

    return (confirm(txtsuretorefund));

}

function checkRefundAbility(v, p, d) {
    //$.facebox('<img src="'+webroot+'facebox/loading.gif">');
    callAjax('index-ajax.php', 'mode=checkforrefund&v=' + v + '&p=' + p + '&d=' + d, function (t) {
        $.facebox(t);
    });
}
...
function checkShippingDetails(v) {
    ...
    jQuery.facebox(function () {
        callAjax('index-ajax.php', 'mode=ShippingDetails&v=' + v, function (t) {
            $.facebox(t);
        });
    });
}
...
function shippingSubmit(frm, v) {
    ...
    callAjax('index-ajax.php', data, function (t) {
...
function digitalProductSendLink(email) {
    ...
    jQuery.facebox(function () {
        callAjax('index-ajax.php', 'mode=sendlinkForm&useremail=' + email, function (t) {
            $.facebox(t);
        });
    });
...
//	$.facebox('<img src="'+webroot+'facebox/loading.gif" alt="Loading...">');
    jQuery.facebox(function () {
        ...
        callAjax('index-ajax.php', 'mode=sendLinkInfoSubmit&email_subject=' + email_subject + '&recipients=' + recipients + '&email_message=' + email_message, function (t) {
            $.facebox(t);
        });
    });
}