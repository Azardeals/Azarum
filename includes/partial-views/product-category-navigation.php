...
global $db;
?>
<a href="<?php echo friendlyUrl(CONF_WEBROOT_URL . 'products.php') ?>"><?php echo t_lang('M_TXT_PRODUCTS'); ?></a>
                    ...
                    <ul class="sublinks">
                        ...
                        if ($rs) {
                            while ($row = $db->fetch($rs)) {
                                ...
                                echo '<li><a href="' . friendlyUrl(CONF_WEBROOT_URL . 'category-deal.php?cat=' . $row['cat_id'] . '&type=product') . '" >' . $row['cat_name' . $_SESSION['lang_fld_prefix']] . '</a>';
                                if ($row['cat_id']) {
                                    echo "<ul>";
                                    if ($res = fetchCategories('product', $row['cat_id'])) {
                                        if (!$res) {
                                            continue;
                                        }
                                        $catCount++;
                                        ...
                                        while ($row1 = $db->fetch($res)) {
                                            $count++;
                                            if ($count > 5) {
                                                echo '<li class="seemore"><a href="' . friendlyUrl(CONF_WEBROOT_URL . 'category-deal.php?cat=' . $row['cat_id'] . '&type=product') . '" >' . t_lang('M_TXT_SEE_MORE') . '</a></li>';
                                                break;
                                            } else {
                                                echo '<li><a href="' . friendlyUrl(CONF_WEBROOT_URL . 'category-deal.php?cat=' . $row1['cat_id'] . '&type=product') . '" >' . $row1['cat_name' . $_SESSION['lang_fld_prefix']] . '</a></li>';
                                            }
                                        }
                                    }
                                    echo "</ul>";
