...
$srch->addMultipleFields(array('company_name' . $_SESSION['lang_fld_prefix'], 'deal_name' . $_SESSION['lang_fld_prefix'], 'SUM(od_deal_price * od_qty) as deal_price', 'SUM(od_deal_tax_amount * od_qty) as tax', 'company_deal_commission_percent'));
if ($post['mode'] == 'search') {
    if ($post['keyword'] != '') {
...
$pagestring = '';
$pagesize = 15;
$pagestring .= createHiddenFormFromPost('frmPaging', '?', array('page'), array('page' => $page, 'status' => $_REQUEST['status']));
            ...
            foreach ($arr_listing_fields as $key => $val) {
                ...
                switch ($key) {
                    ...
                    case 'deal_price':
                        ...
                        break;
                    case 'commission':
                        echo $row['company_deal_commission_percent'];
                        break;
                    case 'tax':
