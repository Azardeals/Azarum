...
$deal_type = array('0-0' => t_lang('M_TXT_DEAL'), '0-1' => t_lang('M_TXT_BOOKING_REQUEST'), '0-2' => t_lang('M_TXT_ONLINE_BOOKING'), '1-0' => t_lang('M_TXT_PRODUCT'), '1-1' => t_lang('M_TXT_DIGITAL_PRODUCT'));
$payment_status = array('-1' => t_lang('M_TXT_CANCELLED'), '0' => t_lang('M_TXT_PENDING'), '1' => t_lang('M_TXT_PAID'), '2' => t_lang('M_TXT_REFUND_SENT'), '3' => t_lang('M_TXT_AUTHORIZED'));
$post = getPostedData();
//Search Form
$status = array('0' => t_lang('M_TXT_PENDING'), '1' => t_lang('M_TXT_SHIPPED'), '2' => t_lang('M_TXT_DELIVERED'));
...
$Src_frm->addSelectBox(t_lang('M_FRM_SHIPPING_STATUS'), 'cm_shipping_status', $status, '', '', 'Select', '');
$Src_frm->addSelectBox(t_lang('M_TXT_TYPE'), 'deal_type', $deal_type, '', '', 'Select', '');
$Src_frm->addDateField(t_lang('M_FRM_ORDER_START_DATE'), 'order_start_time', '', 'order_start_time', '');
$Src_frm->addDateField(t_lang('M_FRM_ORDER_END_DATE'), 'order_end_time', '', 'order_end_time', '');
$defaultDisplay = 1;
$Src_frm->addSelectBox(t_lang('M_FRM_PAYMENT_STATUS'), 'order_payment_status', $payment_status, $defaultDisplay, '', 'Select', '');
$Src_frm->addHiddenField('', 'mode', 'search');
$fld1 = $Src_frm->addButton('', 'btn_search', t_lang('M_TXT_CLEAR_SEARCH'), '', ' class="inputbuttons" onclick=location.href="tipped-members.php"');
$fld = $Src_frm->addSubmitButton('', 'btn_cancel', t_lang('M_TXT_SEARCH'), '', ' class="inputbuttons"');
...
$arr_bread = array(
    ...
    '' => t_lang('M_TXT_VOUCHER')
);
if(!empty($_GET['used'])) {
    $arr_bread = array(
        'company-deals.php' => t_lang('M_TXT_DEALS_PRODUCTS'),
        'tipped-members.php' => t_lang('M_TXT_VOUCHER'), 
...
<ul class="nav-left-ul">
    ...
    <li><a href="tipped-members.php?status=refunded<?php echo $deal_specific; ?>" <?php if ($_GET['status'] == 'refunded') echo 'class="selected"'; ?>><?php echo t_lang('M_TXT_REFUNDED'); ?> </a></li>
...
<td class="right-portion"><?php echo getMerchantBreadCrumb($arr_bread); ?>
    ...
    <?php } else { ?>
        <div class="box searchform_filter"><div class="title"> <?php echo t_lang('M_TXT_TIPPED_MEMBERS_LISTING'); ?> </div><div class="content togglewrap" style="display:none;">	<?php echo $Src_frm->getFormHtml(); ?></div>	 </div>
                            ...
                            if ($row['order_payment_status'] != -1 && $row['order_payment_status'] != 0) {
                                ...
                                if ($row['active'] == 1 && $row['deal_type'] != 1) {
                                    if ($row['canUse'] == 1 && $row['is_tipped']) {
                                        ...
                                        echo '<li><a href="?deal_id=' . $_GET['deal_id'] . '&used=' . $row['cm_id'] . '&page=' . $_GET['page'] . '&status=' . $_REQUEST['status'] . '" title="' . t_lang('M_TXT_MARK_USED') . $title . '" onclick="requestPopup(this,\'' . t_lang('M_MSG_COUPON_CHANGE_STATUS_TO_USE') . '\',1);"><i class="ion-android-checkbox-outline icon"></i>';
                                        echo '</a></li>';
                                        ...
                                        if ($row['deal_tip'] == 1) {
                                            ...
                                            if ($count < $row['deal_tip']) {
                                                echo '<li><a href="javascript:void(0);" onclick="requestPopup(this,\'' . t_lang('M_TXT_DEAL_IS_NOT_TIPPED_YET') . '\',1)" title="*' . t_lang('M_TXT_MARK_USED') . '"><i class="ion-alert icon"></i></a></li>';
                                            } else {
                                                echo '<li> <a   href="?deal_id=' . $_GET['deal_id'] . '&used=' . $row['cm_id'] . '&tip=1&page=' . $_GET['page'] . '&status=' . $_REQUEST['status'] . '" title="*' . t_lang('M_TXT_MARK_USED') . '" onclick="requestPopup(this,\' ' . t_lang('M_MSG_COUPON_CHANGE_STATUS_TO_USE') . '\',1);"><i class="ion-android-checkbox-outline icon"></i></a></li>';
                                            }
                                        }
