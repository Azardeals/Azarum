...
define("EmailXMailer", "MAXIXX-NOTIFICATION (Fatbit Techonologies)");
...
//** FALSE is returned. 
    function Attach($pathtofile, $mimetype = null, $attachmentName = null)
    {
...
//** exist the attachment is not created and FALSE is returned. 
        $attachment = new EmailAttachment($pathtofile, $mimetype, $attachmentName);
...
//** attempt to send the email message. Return the operation success. 
        return mail($this->To, htmlspecialchars_decode($this->Subject), $thebody, $headers);
    }
...
//** type is given the default 'application/octet-stream' is used. 
    function EmailAttachment($pathtofile = null, $mimetype = null, $fName = null)
    {
...
//** Otherwise use the MIME type provided. 
        ...
        if ($fName == null) {
            $this->attachementName = basename($this->FilePath);
        } else {
            $this->attachementName = $fName;
        }
    }
...
//** if an attachment then add the appropriate disposition and file name(s).
        if (!$this->HasLiteralContent()) {
            $header .= " name=\"" . $this->attachementName . "\"" . EmailNewLine .
